...
const kHomeUrl = 'https://app.tinselcrm.com/login';
...
class _HomePageState extends State<HomePage> {
  List<InAppWebViewController?> _controller = [];
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: showWebViewTabsViewer ? _buildWebViewTabViewerAppBar() : _buildWebViewTabAppBar(),
        bottomNavigationBar: BottomAppBar(
          child: Row(
            ...
            children: [
              FutureBuilder<bool>(
                future: webViewTabs[currentTabIndex].canGoBack(),
              ...
              IconButton(
                ...
                onPressed: () {
                  if (_controller.isNotEmpty) {
                    _controller[currentTabIndex]!.reload();
                  }
                },
              ),
              FutureBuilder<bool>(
                future: webViewTabs[currentTabIndex].canGoForward(),
                builder: (context, snapshot) {
                  ...
                  return IconButton(
                    ...
                    onPressed: !canGoForward
                        ? null
                        : () {
                            webViewTabs[currentTabIndex].goForward();
                          },
                  );
                },
              ),
          ...
          Row(
            ...
            children: [
              webViewTabs[currentTabIndex].isSecure != null
                  ? Icon(webViewTabs[currentTabIndex].isSecure == true ? Icons.lock : Icons.lock_open,
                      color: webViewTabs[currentTabIndex].isSecure == true ? Colors.green : Colors.red, size: 12)
                  : Container(),
              const SizedBox(
                width: 5,
              ),
              Flexible(
                  child: Text(
                webViewTabs[currentTabIndex].currentUrl ?? webViewTabs[currentTabIndex].url ?? '',
              ...
              ListTile(
                ...
                title: Row(mainAxisSize: MainAxisSize.max, children: [
                  Container(
                    ...
                    child: favicon != null ? CustomImage(url: favicon.url, maxWidth: 20.0, height: 20.0) : null,
                  ),
                  Expanded(
                      child: Text(
                    webViewTab.title ?? '',
                ...
                trailing: IconButton(
                    onPressed: () {
                      _closeWebViewTab(webViewTab);
                      _controller.remove(webViewIndex);
                    },
                    icon: const Icon(
                      Icons.close,
                      size: 16,
